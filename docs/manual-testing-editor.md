...
- [x] **External setValue shrinking content.** Open the same entry in a second
...
- [x] **Undo after typing several lines.** Type 4–5 new lines into an entry,
      then press your undo shortcut until they're gone. The box shrinks back
      down as each line is removed; it doesn't stay tall.
- [x] **Typing or pasting over a multi-line selection.** Select several lines
...
- [x] **Multi-line paste into short content.** Reverse case: paste several
      lines into a one-line entry. The box grows immediately to fit.

### IME

- [x] **IME composition.** Using an IME (Japanese/Chinese/Korean input, or any
...
- [x] **Divider drag re-wraps text.** With an entry whose text wraps across
...
- [x] **Mounting while the tab is in the background, then switching back.**
...
tuned a fourth time. That makes this a single empirical question rather than a
mechanism to verify — answered by the check below, and recorded in `CLAUDE.md`
§ MVP Definition:

- [x] **Type while an external write lands on the same file.** Open an
...
- [x] **Typing continuously past the autosave debounce drops nothing, with no
...
- [x] **Live preview.** Markdown syntax (headings, bold/italic, lists, code
...
- [x] **No title or properties panel visible.** — verified 19–20 Aug 2026 from timeline screenshots: entries render as timestamp plus body only, with no inline title and no properties panel, on the default light theme. Open an entry with several
      ...
      timestamp.
- [x] **The entry cannot be renamed from the timeline.** Confirm there is no
